...
try {
    if ($method === 'GET') {
        $stmt = $pdo->query("SELECT section_key, section_name, section_content, status FROM benefits_sections ORDER BY id ASC");
    ...
    if ($method === 'POST') {
        ...
        // --- File Cleanup Logic ---
        $stmtOld = $pdo->prepare("SELECT section_content FROM benefits_sections WHERE section_key = ?");
        $stmtOld->execute([$key]);
        ...
        $stmt = $pdo->prepare("UPDATE benefits_sections SET section_content = ?, updated_at = CURRENT_TIMESTAMP WHERE section_key = ?");
